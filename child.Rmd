...
params:
  ...
  template: "Template/input_template.xlsx"
output: 
...
# a list of EFA models (ordered by wave)
model_efa <- split.list(model_efa, 1)
...
df_efa <- sup.cat(map_dfr(lapply(model_efa, get.eigenvalue),bind_rows))
...
if(length(unique(sapply(sup.cat(lapply(model_cfa, get.est)), nrow))) > 1){
