...
params:
  ...
  template: "Template/input_template_lebanon_cs.xlsx"
output: 
...
user_text <- openxlsx::read.xlsx(params$template, sheet = 1)
user_subscale <- openxlsx::read.xlsx(params$template, sheet = 2)
user_model <- openxlsx::read.xlsx(params$template, sheet = 3)

# using assign() causes problems in knitting two documents at the same time, therefore I am manually assigning variable names here
year <- user_text$year
data_file_path <- user_text$data_file_path
model_file_path <- user_text$model_file_path
...
dat <- if(grepl(".dta$", user_text$data_file_path)){
  haven::read_dta(data_file_path) %>% haven::zap_labels()
} else if(grepl(".xlsx$", user_text$data_file_path)){
  openxlsx::read.xlsx(data_file_path)
} else if(grepl(".csv$", duser_text$data_file_path)){
  read_csv(user_text$data_file_path)
}
...
df_cor <- sup.cat(get.cor(model = model_inv_lg[3], # the third one is always the scalar model
...
#### EFA and CFA
```{r efa_screeplot, fig.cap = "EFA model screeplots at all waves", fig.id = "efa_screeplot", fig.height = 4, fig.width = 9, dpi = 300}
...
df_cfa <- sup.cat(map_dfr(lapply(model_cfa, get.fit, path = model_file_path),bind_rows))
...
total_stats <- map_dfr(total_stats, bind_rows) %>% 
  ...
  left_join(sup.cat(get.omega(model_inv_lg[3],path = model_file_path)), by = "subscale_wave") %>% # merge in omega squared 
...
df_inv_tx <- sup.cat(map_dfr(lapply(model_inv_tx, get.invariance, path = model_file_path), bind_rows))
...
df_inv_gender <- sup.cat(map_dfr(lapply(model_inv_gender, get.invariance, path = model_file_path), bind_rows))
...
df_inv_age <- sup.cat(map_dfr(lapply(model_inv_age, get.invariance, path = model_file_path), bind_rows))
...
df_inv_lg <- sup.cat(get.invariance(model_inv_lg, path = model_file_path))
