...
select_var_data <- BBmisc::convertColsToList(user_select_var)

select_var_data <- as.character(
  unlist(
...
`std_alpha`    : The standarized alpha based upon the correlations                                                                           
...
alpha_drop <- purrr::map_dfr(alpha_drop, dplyr::bind_rows) %>% 
  ...
  rename(alpha_se = `alpha se`, var_r = `var.r`, median_r = `med.r`,
         std_alpha = `std.alpha`)
...
total_stats <- purrr::map_dfr(total_stats, dplyr::bind_rows) %>% 
  ...
  rename(alpha_se = `ase`, std_alpha = `std.alpha`)
