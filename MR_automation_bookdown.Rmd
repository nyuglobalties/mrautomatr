...
df_cor <- get.cor(model = model_lg_inv[[3]], # the third item should be the scalar model
             string = ".WITH$")
...
total_stats <- map_dfr(total_stats, bind_rows) %>% 
            round(digits = 3) %>%
            mutate(subscale = unique(subscale_item_name$subscale)) %>%
  left_join(get.omega(model_lg_inv[[3]]), by = "subscale") %>% # merge in omega squared 
  select(subscale, omega, everything())

flextable(total_stats) %>%
  align(part = "all") %>%
  font(fontname = "Times", part = "all") %>% 
  bold(part = "header") %>%
  fontsize(size = 10, part = "body") %>% 
...
df_inv_tx <- map_dfr(lapply(model_inv_tx, get.invariance), bind_rows)
...
df_inv_gender <- map_dfr(lapply(model_inv_gender, get.invariance), bind_rows)

flextable(df_inv_gender) %>% 
  compose(
    part = "header", j = "ChiSqM_Value",
    value = as_paragraph("\U03C7",as_sup("2"))
    ) %>%
...
df_inv_age <- map_dfr(lapply(model_inv_age, get.invariance), bind_rows)

flextable(df_inv_age) %>% 
  compose(
    part = "header", j = "ChiSqM_Value",
    value = as_paragraph("\U03C7",as_sup("2"))
    ) %>%
...
df_inv_lg <- get.invariance(model_lg_inv)
