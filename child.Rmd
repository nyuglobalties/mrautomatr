...
```{r correlation_matrix_lg, tab.cap = "Factor correlations from the longitudinal invariance models", tab.id = "correlation_matrix_lg"}
...
```{r correlation_matrix_bivar, tab.cap = "Bivariate correlations among factors", tab.id = "correlation_matrix_bivar"}
...
```{r correlation_matrix_item, tab.cap = "Bivariate correlations among all items", tab.id = "correlation_matrix_item", eval = F}
...
total_stats <- map_dfr(total_stats, bind_rows) %>% 
  ...
  select(subscale, wave, omega_lg, omega_by_wave, everything(), -subscale_wave)

if(sum(is.na(total_stats$omega_lg)) > 0){
  warning("Subscales or subscale names are different in CFA and in the longitudinal invariance model. Please double check.")
}

if(sum(is.na(total_stats$omega_by_wave)) > 0){
  warning("Subscales or subscale names are different in CFA and in the input file template. Please double check.")
}
